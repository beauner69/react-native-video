...
- (RCTVideoPlayerViewController *)createPlayerViewController:(AVPlayer *)player
  ...
  RCTVideoPlayerViewController *viewController =
...
- (void)addPlayerTimeObserver {
  ...
  _timeObserver =
      [_player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(
...
- (void)applicationDidEnterBackground:(NSNotification *)notification {
  if (_playInBackground) {
    // Needed to play sound in background. See
...
- (void)audioRouteChanged:(NSNotification *)notification {
  NSNumber *reason =
      [[notification userInfo] objectForKey:AVAudioSessionRouteChangeReasonKey];
  NSNumber *previousRoute = [[notification userInfo]
      objectForKey:AVAudioSessionRouteChangePreviousRouteKey];
  if (reason.unsignedIntValue ==
...
- (void)sendProgressUpdate {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"RCTVideo_progress"
                    object:nil
                  userInfo:@{
                    @"progress" :
                        [NSNumber numberWithDouble:currentTimeSecs / duration]
                  }];
...
/*!
 * Calculates and returns the playable duration of the current player item using
 * its loaded time ranges.
 *
...
- (NSNumber *)calculatePlayableDuration {
  ...
  if (video.status == AVPlayerItemStatusReadyToPlay) {
    ...
    Float64 playableDuration =
        CMTimeGetSeconds(CMTimeRangeGetEnd(effectiveTimeRange));
...
- (void)addPlayerItemObservers {
  ...
  [_playerItem addObserver:self
                forKeyPath:playbackBufferEmptyKeyPath
                   options:0
                   context:nil];
  [_playerItem addObserver:self
                forKeyPath:playbackLikelyToKeepUpKeyPath
                   options:0
                   context:nil];
  [_playerItem addObserver:self
...
- (NSURL *)urlFilePath:(NSString *)filepath {
  ...
  NSArray *fileComponents =
      [filepath componentsSeparatedByString:@"Documents/"];
  if (fileComponents.count > 1) {
    relativeFilePath = [fileComponents objectAtIndex:1];
  }

  NSString *path =
      [paths.firstObject stringByAppendingPathComponent:relativeFilePath];
...
- (void)playerItemPrepareText:(AVAsset *)asset
  ...
  for (int i = 0; i < _textTracks.count; ++i) {
    ...
    if ([[textUri lowercaseString] hasPrefix:@"http"]) {
      textURLAsset = [AVURLAsset URLAssetWithURL:[NSURL URLWithString:textUri]
                                         options:assetOptions];
    } else {
      textURLAsset =
          [AVURLAsset URLAssetWithURL:[self urlFilePath:textUri] options:nil];
    }
    AVAssetTrack *textTrackAsset =
        [textURLAsset tracksWithMediaType:AVMediaTypeText].firstObject;
    if (!textTrackAsset)
      continue; // fix when there's no textTrackAsset
...
- (void)playerItemForSource:(NSDictionary *)source
  ...
  NSURL *url =
      isNetwork || isAsset
          ...
          : [[NSURL alloc] initFileURLWithPath:[[NSBundle mainBundle]
                                                   pathForResource:uri
                                                            ofType:type]];
  ...
  if (isNetwork) {
     ...
     * We need to either get approval from Apple to use this  or use a different
     approach. NSDictionary *headers = [source objectForKey:@"requestHeaders"];
     if ([headers count] > 0) {
...
#if __has_include(<react-native-video/RCTVideoCache.h>)
    if (!_textTracks) {
      /* The DVURLAsset created by cache doesn't have a tracksWithMediaType
       * property, so trying to bring in the text track code will crash. I
       * suspect this is because the asset hasn't fully loaded. Until this is
       * fixed, we need to bypass caching when text tracks are specified.
       */
      DebugLog(@"Caching is not supported for uri '%@' because text tracks are "
               @"not compatible with the cache. Checkout "
               @"https://github.com/react-native-community/react-native-video/"
               @"blob/master/docs/caching.md",
               uri);
      [self playerItemForSourceUsingCache:uri
...
- (void)playerItemForSourceUsingCache:(NSString *)uri
                         assetOptions:(NSDictionary *)options
                         withCallback:(void (^)(AVPlayerItem *))handler {
  NSURL *url = [NSURL URLWithString:uri];
  [_videoCache
      getItemForUri:uri
       withCallback:^(RCTVideoCacheStatus videoCacheStatus,
                      AVAsset *_Nullable cachedAsset) {
         switch (videoCacheStatus) {
         case RCTVideoCacheStatusMissingFileExtension: {
           DebugLog(
               @"Could not generate cache key for uri '%@'. It is currently "
               @"not supported to cache urls that do not include a file "
               @"extension. The video file will not be cached. Checkout "
               @"https://github.com/react-native-community/react-native-video/"
               @"blob/master/docs/caching.md",
               uri);
         ...
         case RCTVideoCacheStatusUnsupportedFileExtension: {
           DebugLog(@"Could not generate cache key for uri '%@'. The file "
                    @"extension of that uri is currently not supported. The "
                    @"video file will not be cached. Checkout "
                    @"https://github.com/react-native-community/"
                    @"react-native-video/blob/master/docs/caching.md",
                    uri);
           AVURLAsset *asset = [AVURLAsset URLAssetWithURL:url options:options];
           [self playerItemPrepareText:asset
                          assetOptions:options
         ...
         default:
           if (cachedAsset) {
             ...
             // See note in playerItemForSource about not being able to support
             // text tracks & caching
             handler([AVPlayerItem playerItemWithAsset:asset]);
             return;
           }
         ...
         DVURLAsset *asset = [[DVURLAsset alloc] initWithURL:url
                                                     options:options
                                              networkTimeout:10000];
         asset.loaderDelegate = self;

         /* More granular code to have control over the DVURLAsset
         DVAssetLoaderDelegate *resourceLoaderDelegate = [[DVAssetLoaderDelegate
         alloc] initWithURL:url]; resourceLoaderDelegate.delegate = self;
         NSURLComponents *components = [[NSURLComponents alloc] initWithURL:url
         resolvingAgainstBaseURL:NO]; components.scheme = [DVAssetLoaderDelegate
         scheme]; AVURLAsset *asset = [[AVURLAsset alloc]
         initWithURL:[components URL] options:options]; [asset.resourceLoader
         setDelegate:resourceLoaderDelegate queue:dispatch_get_main_queue()];
         */
...
- (void)dvAssetLoaderDelegate:(DVAssetLoaderDelegate *)loaderDelegate
                  didLoadData:(NSData *)data
                       forURL:(NSURL *)url {
  [_videoCache storeItem:data
                  forUri:[url absoluteString]
  ...
  if (object == _playerItem) {
        ...
        for (AVMetadataItem *item in items) {
          ...
          if (![value isEqual:[NSNull null]]) {
            NSDictionary *dictionary = [[NSDictionary alloc]
                initWithObjects:@[ value, identifier ]
                        forKeys:@[ @"value", @"identifier" ]];

            [array addObject:dictionary];
          }
        }

        self.onTimedMetadata(@{@"target" : self.reactTag, @"metadata" : array});
      }
    }
    ...
    if ([keyPath isEqualToString:statusKeyPath]) {
      ...
      if (_playerItem.status == AVPlayerItemStatusReadyToPlay) {
        ...
        if ([_playerItem.asset tracksWithMediaType:AVMediaTypeVideo].count >
            0) {
          AVAssetTrack *videoTrack = [[_playerItem.asset
              tracksWithMediaType:AVMediaTypeVideo] objectAtIndex:0];
        ...
        if (self.onVideoLoad && _videoLoadStarted) {
          self.onVideoLoad(@{
            ...
            @"currentTime" : [NSNumber
                numberWithFloat:CMTimeGetSeconds(_playerItem.currentTime)],
            @"canPlayReverse" :
                [NSNumber numberWithBool:_playerItem.canPlayReverse],
            @"canPlayFastForward" :
                [NSNumber numberWithBool:_playerItem.canPlayFastForward],
            @"canPlaySlowForward" :
                [NSNumber numberWithBool:_playerItem.canPlaySlowForward],
            @"canPlaySlowReverse" :
                [NSNumber numberWithBool:_playerItem.canPlaySlowReverse],
            @"canStepBackward" :
                [NSNumber numberWithBool:_playerItem.canStepBackward],
            @"canStepForward" :
                [NSNumber numberWithBool:_playerItem.canStepForward],
            @"naturalSize" : @{
              @"width" : width,
              @"height" : height,
      ...
      // Continue playing (or not if paused) after being paused due to hitting
      // an unbuffered zone.
      if ((!(_controls || _fullscreenPlayerPresented) || _playerBufferEmpty) &&
    ...
    if ([keyPath isEqualToString:readyForDisplayKeyPath] &&
    ...
    if ([keyPath isEqualToString:playbackRate]) {
      if (self.onPlaybackRateChange) {
        self.onPlaybackRateChange(@{
          @"playbackRate" : [NSNumber numberWithFloat:_player.rate],
          @"target" : self.reactTag
        });
      }
      ...
      if (self.onVideoExternalPlaybackChange) {
        self.onVideoExternalPlaybackChange(@{
          @"isExternalPlaybackActive" :
              [NSNumber numberWithBool:_player.isExternalPlaybackActive],
          @"target" : self.reactTag
        });
      }
    }
      ...
      [_player seekToTime:cmSeekTime
          ...
          completionHandler:^(BOOL finished) {
            ...
            if (self.onVideoSeek) {
              self.onVideoSeek(@{
                @"currentTime" : [NSNumber
                    numberWithFloat:CMTimeGetSeconds(item.currentTime)],
                @"seekTime" : seekTime,
                @"target" : self.reactTag
              });
            }
...
- (void)setMediaSelectionTrackForCharacteristic:
            (AVMediaCharacteristic)characteristic
  ...
  if ([type isEqualToString:@"disabled"]) {
    ...
    for (int i = 0; i < group.options.count; ++i) {
      ...
      if ([type isEqualToString:@"language"]) {
        ...
        optionValue = [[[currentOption commonMetadata] valueForKey:@"value"]
            objectAtIndex:0];
      }
  ...
  [_player.currentItem selectMediaOption:mediaOption
...
- (void)setSideloadedText {
  ...
  for (firstTextIndex = 0; firstTextIndex < _player.currentItem.tracks.count;
       ++firstTextIndex) {
    if ([_player.currentItem.tracks[firstTextIndex].assetTrack
            hasMediaCharacteristic:AVMediaCharacteristicLegible]) {
      break;
    }
  }
  ...
  // a textTrack not available)
  if (![type isEqualToString:@"disabled"] &&
      selectedTrackIndex == RCTVideoUnset) {
    CFArrayRef captioningMediaCharacteristics =
        MACaptionAppearanceCopyPreferredCaptioningMediaCharacteristics(
            kMACaptionAppearanceDomainUser);
    NSArray *captionSettings =
        (__bridge NSArray *)captioningMediaCharacteristics;
    if ([captionSettings
            containsObject:
                AVMediaCharacteristicTranscribesSpokenDialogForAccessibility]) {
      selectedTrackIndex =
          0; // If we can't find a match, use the first available track
...
- (NSArray *)getAudioTrackInfo {
  ...
  for (int i = 0; i < group.options.count; ++i) {
    ...
    NSString *language = [currentOption extendedLanguageTag]
...
- (void)setFullscreen:(BOOL)fullscreen {
  if (fullscreen && !_fullscreenPlayerPresented && _player) {
    ...
    if (viewController) {
      ...
      [viewController presentViewController:_playerViewController
                                   animated:true
                                 completion:^{
...
- (void)usePlayerViewController {
  if (_player) {
    _playerViewController =
        [self createPlayerViewController:_player withPlayerItem:_playerItem];
...
- (void)videoPlayerViewControllerWillDismiss:
    (AVPlayerViewController *)playerViewController {
  if (_playerViewController == playerViewController &&
      _fullscreenPlayerPresented && self.onVideoFullscreenPlayerWillDismiss) {
    self.onVideoFullscreenPlayerWillDismiss(@{@"target" : self.reactTag});
  }
}

- (void)videoPlayerViewControllerDidDismiss:
    (AVPlayerViewController *)playerViewController {
  if (_playerViewController == playerViewController &&
...
- (void)insertReactSubview:(UIView *)view atIndex:(NSInteger)atIndex {
  ...
  if (_controls) {
    ...
    [_playerViewController.contentOverlayView insertSubview:view
                                                    atIndex:atIndex];
  } else {
