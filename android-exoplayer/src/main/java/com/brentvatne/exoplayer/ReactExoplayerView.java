...
class ReactExoplayerView extends FrameLayout
    implements LifecycleEventListener, ExoPlayer.EventListener,
               BecomingNoisyListener, AudioManager.OnAudioFocusChangeListener,
      ...
      case SHOW_PROGRESS:
        if (player != null &&
          ...
          long bufferedDuration =
              player.getBufferedPercentage() * player.getDuration() / 100;
          eventEmitter.progressChanged(pos, bufferedDuration,
                                       player.getDuration());
  ...
  public ReactExoplayerView(ThemedReactContext context) {
    ...
    audioBecomingNoisyReceiver =
        new AudioBecomingNoisyReceiver(themedReactContext);
  ...
  public void cleanUpResources() { stopPlayback(); }
  ...
  private ArrayList<MediaSource> buildTextSources() {
    ...
    for (int i = 0; i < textTracks.size(); ++i) {
      ...
      String title = textTrack.hasKey("title") ? textTrack.getString("title")
                                               : language + " " + i;
      Uri uri = Uri.parse(textTrack.getString("uri"));
      MediaSource textSource =
          buildTextSource(title, uri, textTrack.getString("type"), language);
      if (textSource != null) {
  ...
  private MediaSource buildTextSource(String title, Uri uri, String mimeType,
                                      String language) {
    Format textFormat = Format.createTextSampleFormat(
        title, mimeType, Format.NO_VALUE, language);
    return new SingleSampleMediaSource(uri, mediaDataSourceFactory, textFormat,
                                       C.TIME_UNSET);
  }
  ...
  private DataSource.Factory buildDataSourceFactory(boolean useBandwidthMeter) {
    return DataSourceUtil.getDefaultDataSourceFactory(
        this.themedReactContext, useBandwidthMeter ? BANDWIDTH_METER : null,
        requestHeaders);
  }
  ...
  public void onPlayerStateChanged(boolean playWhenReady, int playbackState) {
    String text =
        "onStateChanged: playWhenReady=" + playWhenReady + ", playbackState=";
  ...
  private void videoLoaded() {
    if (loadVideoStarted) {
      ...
      eventEmitter.load(player.getDuration(), player.getCurrentPosition(),
                        width, height, getAudioTrackInfo(), getTextTrackInfo());
    }
  }
  ...
  public void onPositionDiscontinuity(int reason) {
    if (playerNeedsSource) {
      // This will only occur if the user has performed a seek whilst in the
      // error state. Update the resume position so that if the user then
      // retries, playback will resume from the position to which they seeked.
      updateResumePosition();
    }
    // When repeat is turned on, reaching the end of the video will not cause a
    // state change so we need to explicitly detect it.
  ...
  public int getTrackRendererIndex(int trackType) {
    ...
    for (int rendererIndex = 0; rendererIndex < rendererCount;
         rendererIndex++) {
  ...
  public void setRawSrc(final Uri uri, final String extension) {
    if (uri != null) {
      ...
      this.mediaDataSourceFactory =
          DataSourceUtil.getRawDataSourceFactory(this.themedReactContext);
  ...
  public void setSelectedTrack(int trackType, String type, Dynamic value) {
    ...
    DefaultTrackSelector.Parameters disableParameters =
    ...
    if (type.equals("disabled")) {
      ...
      for (int i = 0; i < groups.length; ++i) {
        ...
        if (format.language != null &&
            format.language.equals(value.asString())) {
          trackIndex = i;
          break;
        }
      }
      ...
      if (rendererIndex == C.TRACK_TYPE_TEXT && Util.SDK_INT > 18 &&
          groups.length > 0) {
        // Use system settings if possible
        CaptioningManager captioningManager =
            (CaptioningManager)themedReactContext.getSystemService(
  ...
  private int getTrackIndexForDefaultLocale(TrackGroupArray groups) {
    if (groups.length ==
        0) { // Avoid a crash if we try to select a non-existant group
      return C.INDEX_UNSET;
    }
    ...
    for (int i = 0; i < groups.length; ++i) {
      ...
      if (language != null &&
          (language.equals(locale2) || language.equals(locale3))) {
        trackIndex = i;
        break;
      }
    }
  ...
  public void setBufferConfig(int newMinBufferMs, int newMaxBufferMs,
                              int newBufferForPlaybackMs,
